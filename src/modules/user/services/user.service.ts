...
export class UserService {
  ...
  public async putUser(requestUser: UserEntity, userId: string, payload: UpdateUserPayload): Promise<UserEntity> {
    ...
    if (requestUser.id !== user.id) throw new ForbiddenException('Você não tem permissão para atualizar esse usuário');
  ...
  public async deleteUser(requestUser: UserEntity, userId: string,): Promise<void> {
    ...
    if (requestUser.id !== user.id) throw new ForbiddenException('Você não tem permissão para deletar esse usuário');
